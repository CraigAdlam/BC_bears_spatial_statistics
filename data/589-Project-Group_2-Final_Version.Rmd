...
author: "Craig Adlam, Kulaphong Jitareerat, Nijiati Abulizi"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
### Load Park Locations Data, Covariate Data, and Black Bear Occurrences Data
```{r}
...
### Plot the Poisson Point Pattern (ppp) Object
...
### Visualize the Quadrat Count Plot and Resulting Distribution
```{r fig.height=6, fig.width=8}
...
Since the intensity is inhomogeneous, identifying areas of elevated intensity (i.e., hotspots) is important to gain valuable information about the black bear occurrences spatial processes. After using kernel estimates plots above, there are clear regions of interest near the coastal edge in the southern tip of BC. However, an objective test can be carried out using the scan test, which utilizes the likelihood ratio. Scan tests function as such: at each location $u$, a circle is drawn of radius $r$ and the number of points in $n_{\text{in}} = n(x \cap b(u,r))$ and out $n_{\text{out}} = n(x \cap W \notin b(u,r))$ of the circle are counted. Under an assumption that the process is Poisson distributed, likelihood ratio test statistics for the number of points inside vs. outside of the circle can be calculated. The null distribution is $\sim \chi^2$ with 1 degree of freedom, allowing p-values at each location $u$ to be calculated. This test can be undertaken via the `scanLRTS()` function.

The choice of the radius $r$ was obtained through kernel bandwidth optimisation above (`bw_ppl`).
```{r fig.height=6, fig.height=8}
# Estimate of R is above in the bw_ppl object

#Calculate test statistic
likelihood_ratio <- scanLRTS(ppp_bears, r = bw_ppl)
```

### Visualize the likelihood output and p-values of Specific Areas of Interest (< 0.2)
```{r fig.height=6, fig.width=8}
#Plot the likelihood output 
plot(likelihood_ratio, main = "Location(s) of Elevated Intensity")

#Plot the points --- MADE SMALLLER TO SEE BEAR OCCURRENCES, CAN COMMENT OUT
plot(ppp_bears,
     pch = 20,
     cex = 0.5,
     cols = "grey30",
...
#Plot the output
plot(pvals_bears, main = "Local p-values of 'Hotspots'")

# #Plot the points
# plot(ppp_bears, add = TRUE, cols = "white", pch = 16, cex = 0.6) # CAN COMMENT OUT
# plot(ppp_bears, add = TRUE, cols = "black", pch = 16, cex = 0.5) # CAN COMMENT OUT
...
# Plot the KDE with highest y values
plot(kde_bears, main = "Elevation in BC", col = rgb(0, 0, 1, 1/4), # xlim = c(0, max(kde_bears$x) * 1.1)
     xlab = "Level of Elevation (m)", ylab = "Density")
...
# Add legend
legend("topright", legend = c("Elevations", "Elevations at Bear Occurrences"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
```
...
# Plot the KDE with highest y values
plot(kde_BC, main = "Forest Coverage in BC", col = rgb(0, 0, 1, 1/4), # xlim = c(0, max(kde_bears$x) * 1.1)
     xlab = "Percentage of Forest Coverage", ylab = "Density")
...
# Add legend
legend("topright", legend = c("Forest Coverage", "Forest Coverage at Bear Occurrence"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
```
...
# Plot the first KDE
plot(kde_BC, main = "Human Footprint Index in BC", col = rgb(0, 0, 1, 1/4), # xlim = c(0, max(kde_bears$x) * 1.1)
     xlab = "Human Footprint Index (HFI)", ylab = "Density")
...
# Add legend
legend("topright", legend = c("HFI", "HFI at Bear Occurrence"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
```
...
# Plot the first KDE
plot(kde_BC, main = "Distance from Water in BC", col = rgb(0, 0, 1, 1/4), # xlim = c(0, max(kde_bears$x) * 1.1)
     xlab = "Distance from Water (m)", ylab = "Density")
...
# Add legend
legend("topright", legend = c("Distance from Water", "Distance from Water at Bear Occurrence"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
```

### Visualizing All 4 Density Plots
```{r fig.height=6, fig.width=8}
par(mfrow = c(2,2))
...
# Add legend
legend("topright", legend = c("Forest Coverage", "Forest Coverage at Bear Occurrences"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
...
# Add legend
legend("topright", legend = c("HFI", "HFI at Bear Occurrences"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
...
# Add legend
legend("topright", legend = c("Distance from Water", "Distance from Water at Bear Occurrences"), col = c(rgb(0, 0, 1, 1/4), rgb(1, 0, 0, 1/2)), lty = 1, bty = "n")
```

### Compare the Distribution of Covariate Values Based on 4 Quadrats from Below
```{r fig.height=6, fig.width=8}
library(viridis)

categories <- c("Elevation", "Forest", "HFI", "Dist_Water")
levels <- c("Low", "Moderate-Low", "Moderate-High", "High")
Very_Low <- c(1145, 168, 21, 659)
Low <- c(327, 720, 81, 321)
High <- c(168, 572, 249, 355)
Very_High <- c(83, 257, 1371, 375)

# Combine data into a matrix and transpose it
data_matrix <- t(data.frame(Very_Low, Low, High, Very_High))

# Barplot with viridis color palette
barplot(data_matrix, beside = TRUE, legend.text = levels, col = topo.colors(4),
        main = "Bear Occurrence Quadrat Counts (In Relation to Covariate Information)", xlab = "Covariates", ylab = "Frequency",
...
```{r fig.height=6, fig.width=8}
plot(rho_bears, xlim = c(0, max(rho_bears$elev_bears)), main = "Intensity of Black Bear Occurrences wrt Elevation", xlab = "Level of Elevation (m)", ylab = "p(elevation)")
```

### Interpolate to Explore Relative Differences
```{r}
# Find the intensity at different elevations using linear interpolation
elev_interpolate <- 825
...
cat("Intensity of black bear occurrences at", elev_interpolate, "m:", intensity_interpolate, "\n")
cat("Average intensity of black bear occurrences in BC:", mean(rho_bears$ave), "\n\n")
cat("Intensity of black bear occurrences at", elev_interpolate, "m is", abs(intensity_increase), "%", increase_decrease)
```
...
# Split image into 4 equal-area quadrats based on forest values
...
### Visualize the Intensity of Black Bear Occurrences as a Function of Forest
```{r fig.height=6, fig.width=8}
plot(rho_bears_forest, xlim = c(0, max(rho_bears_forest$forest_bears)), main = "Intensity of Black Bear Occurences wrt Forest", xlab = "Percentage of Forest Coverage", ylab = "p(forest)")
```

### Interpolate to Explore Relative Differences
```{r}
# Find the intensity at different forest coverages using linear interpolation
...
# Display the intensity at defined forest coverage
cat("Intensity of black bear occurrences at", forest_interpolate, "% forest coverage:", intensity_forest, "\n")
cat("Average intensity of black bear occurrences in BC:", mean(rho_bears_forest$ave), "\n\n")
cat("Intensity of black bear occurrences at", forest_interpolate, "% forest coverage is", abs(intensity_increase_forest), "%", increase_decrease_forest)
```
...
```{r}
# Extract HFI information
HFI_bears <- data_covariates$HFI

# Define quartiles
b <- quantile(HFI_bears, probs = (0:4)/4, type = 2)

# Split image into 4 equal-area quadrats based on HFI values
...
### Visualize the HFI Quadrat Results
```{r fig.height=6, fig.width=8}
# plot(quadratcount(ppp_bears, tess = V_HFI))
plot(V_HFI, main = "Black Bear Occurrences wrt HFI")
plot(ppp_bears, add = TRUE, cols = "white", pch = 16, cex = 0.5)
plot(ppp_bears, add = TRUE, cols = "black", pch = 16, cex = 0.3)
```
...
### Visualize the Intensity of Black Bear Occurrences as a Function of HFI
```{r fig.height=6, fig.width=8}
plot(rho_bears_HFI, xlim = c(0, max(rho_bears_HFI$HFI_bears)), main = "Intensity of Black Bear Occurences wrt HFI", xlab = "Human Footprint Index (HFI)", ylab = "p(HFI)")
```
...
plot(V_DIST_WATER, main = "Black Bear Occurrences wrt to Dist_Water")
plot(ppp_bears, add = TRUE, cols = "white", pch = 16, cex = 0.5)
plot(ppp_bears, add = TRUE, cols = "black", pch = 16, cex = 0.3)
```
...
```{r fig.height=6, fig.width=8}
plot(rho_bears_DIST_WATER, xlim = c(0, max(rho_bears_DIST_WATER$DIST_WATER_b)), main = "Intensity of Black Bear Occurences wrt Dist_Water", xlab = "Distance from Water (m)", ylab = "p(Dist_Water)")
```

### Interpolate to Explore Relative Differences
```{r}
# Find the intensity at different distances from water using linear interpolation
...
# Display the intensity at defined distance from water
cat("Intensity of black bear occurrences at", DIST_WATER_interpolate, "HFI:", intensity_DIST_WATER, "\n")
cat("Average intensity of black bear occurrences in BC:", mean(rho_bears_DIST_WATER$ave), "\n\n")
cat("Intensity of black bear occurrences at", DIST_WATER_interpolate, "HFI is", abs(intensity_increase_DIST_WATER), "%", increase_decrease_DIST_WATER)
```
...
### Visualize the Quadrat Results for Elevation and Forest
```{r fig.height=6, fig.width=8}

par(mfrow = c(1,2))

plot(V_elev, main = "Level of Elevation (m)")
plot(ppp_bears, add = TRUE, cols = "white", pch = 20, cex = 0.6)
plot(ppp_bears, add = TRUE, cols = "black", pch = 20, cex = 0.3)

plot(V_forest, main = "Percentage of Forest Coverage")
plot(ppp_bears, add = TRUE, cols = "white", pch = 20, cex = 0.6)
plot(ppp_bears, add = TRUE, cols = "black", pch = 20, cex = 0.3)
```

### Visualize the Quadrat Results for HFI and Dist_Water
```{r fig.height=6, fig.width=8}

par(mfrow = c(1,2))

plot(V_HFI, main = "Human Footprint Index")
plot(ppp_bears, add = TRUE, cols = "white", pch = 20, cex = 0.6)
plot(ppp_bears, add = TRUE, cols = "black", pch = 20, cex = 0.3)

plot(V_DIST_WATER, main = "Distance from Water (m)")
plot(ppp_bears, add = TRUE, cols = "white", pch = 20, cex = 0.6)
plot(ppp_bears, add = TRUE, cols = "black", pch = 20, cex = 0.3)
```
...
# Range of x and y values to be used for rectangle window; c(-1, 1) includes min/max
...
miplot(ppp_bears_bbox,
       # ylim = c(0,333),
       main = "Correlations Based on Rate of Point Pairs Found Close Together",
       pch = 16,
       col = "#046C9A")
```
...
```{r}
# Estimate intensity
lambda_bears <- density.ppp(ppp_bears, bw_ppl) # if code isn't run from top, switch to bw.ppl

# Inhomogeneous K-function
# Estimates the inhomogeneous K-function of a non-stationary point pattern.
Kinhom_bears <- Kinhom(ppp_bears, lambda_bears, correction = "border")
...
lambda_bears_pos <- density.ppp(ppp_bears,
...
par(mfrow = c(1,2))
plot(E_bears_inhom,
     main = "Ripley's K (entire range)",
     lwd = 2)

# Zoom in on range where significant deviations appear
plot(E_bears_inhom,
     xlim = c(0, 60000),
     main = "Area of Positive Deviations",
     lwd = 2)
```
...
### SPLITTING DATA HERE FOR G-FUNCTION
```{r}
...
E_bears_g_func_inhom_plot <- plot(E_bears_g_func_inhom,
                                  main = "Pairs Correlation Function (g-function)",
                                  lwd = 2)
# E_bears_g_func_inhom_plot
```

### Visualize the Results from $g$-function with Confidence Bands Included (ZOOM IN)
```{r fig.height=6, fig.width=8}
# visualise the results
par(mfrow = c(1,2)) # --- MODIFY TO c(1,3) TO VIEW 3RD AREA ---
plot(E_bears_g_func_inhom,
     main = "g-function (entire range)",
     lwd = 2)

# Zoom in on range where significant deviations appear
plot(E_bears_g_func_inhom,
     xlim = c(0, 50000),
     main = "Area of Positive Deviations 1",
     lwd = 2)

# # Zoom in on range where significant deviations appear --- UNCOMMENT FOR 3RD AREA ---
# plot(E_bears_g_func_inhom,
#      xlim = c(245000, 255000),
#      main = "Area of Positive Deviations 2",
#      lwd = 2)
```

### Visualize the Positive Deviations from $g$-function curve with Confidence Bands Included (ZOOM IN)
```{r fig.height=6, fig.width=8}
# visualise the results
par(mfrow = c(1,3))
plot(E_bears_g_func_inhom,
...
### Visualize the Negative Deviations from $g$-function curve with Confidence Bands Included (ZOOM IN)
```{r fig.height=6, fig.width=8}
# visualise the results
par(mfrow = c(1,3))
plot(E_bears_g_func_inhom,
     main = "Pairs Correlation Function (entire range)",
     lwd = 2)

# Zoom in on range where significant deviations appear
plot(E_bears_g_func_inhom,
     xlim = c(130000, 160000),
     main = "Area of Negative Deviations 1",
     lwd = 2)

# Zoom in on range where significant deviations appear
plot(E_bears_g_func_inhom,
     xlim = c(250000, 350000),
     main = "Area of Negative Deviations 2",
     lwd = 2)
```
...
### Comparing Bear Occurrences with BC Park Locations
```{r}
...
### Visualize the Point Patterns of Bear Occurrences as Well as Park Locations
```{r fig.height=6, fig.width=8}
# Plot the point pattern & assign to variable for specific information
ppp_parks_plot <- plot(ppp_parks,
                       main = "Point Pattern of Parks and Bear Occurrences", # Add title to plot to be more descriptive
                       col = "grey90", # Make the background slightly darker for better visibility
...
legend("topright", 
       legend = c("North", "Ok", "South", "Tc", "West", "Bear"), 
       title = "Marks", 
       col = c(brewer.pal(n = 5, name = "Dark2"), "red"), 
       pch = c(15, 19, 18, 17, 16, 20), 
       cex = 0.8
)

# Plot the bear data points
plot(ppp_bears, add = TRUE, cols = "black", pch = 20, cex = 0.6)
plot(ppp_bears, add = TRUE, cols = "red", pch = 20, cex = 0.3)
```

### Using Quadrat Counts, Compare Bear Occurrences and Park Locations
```{r}
# Split into a 8 by 8 quadrat to match ppp_bear point pattern quadrats
Q_parks <- quadratcount(ppp_parks,
                        nx = 8,
                        ny = 8)
# Q_parks

# Quadrat test of homogeneity 
quadrat.test(Q_parks)
```

Both bear occurrences and park locations are inhomogenous based on the $\chi^2$ test.

### Visualize and Compare Bear Occurrences and Park Locations via Quadrats
```{r fig.height=6, fig.width=8}
par(mfrow = c(1,2))
...
# Plot the output Note the use of image = TRUE
plot(intensity(Q_parks, image = T),
     main = "Park Locations (Quadrats)")
...
### Visualize and Compare Bear Occurrences with Park Locations via Kernel Estimates
```{r fig.height=6, fig.width=8}
par(mfrow = c(1,2))
...
# Density estimation of lambda(u)
lambda_u_hat_parks <- density.ppp(ppp_parks)

# Plot the output Note the use of image = TRUE
plot(lambda_u_hat_parks,
     main = "Park Locations (Kernel Est.)")
...
### Cross-Validated Likelihood of Park Locations Point Pattern
```{r}
# Use the likelihood CV by itself for plotting after (then can modify plotting)
bw_ppl_parks <- bw.ppl(ppp_parks)
```
...
plot(density.ppp(ppp_bears, sigma = bw_ppl), # Likelihood Cross Validation Bandwidth Selection
     ribbon = F,
     main = "Bear Occurrences (Likelihood CV)")
#   sigma
# 15768.64
...
# PARKS BELOW HERE
plot(density.ppp(ppp_parks, sigma = bw_ppl_parks), # Likelihood Cross Validation Bandwidth Selection
     ribbon = F,
     main = "Park Locations (Likelihood CV)")
...
### Comparing Distributions of the Two Common Covariates Between Parks and Bear Occurrences
...
# Plot the KDE with the largest y value
plot(kde_bears, main = "Elevations in BC", 
     col = rgb(0, 0, 1, 1/2),
...
# Plot the KDE with the largest y value first
plot(kde_parks, main = "Forest Coverage in BC",
     col = rgb(0, 0, 1, 1/2),
...
```{r}
# # Compute the kernel density estimate for park forests --- USE IF WANT TO SEPARATE FROM ABOVE ---
# kde_parks <- density(as.numeric(data_parks$Forest[data_parks$Parks]))
# 
# # Compute the kernel density estimate for bear forests
# kde_bears <- density(as.numeric(data_covariates$Forest[ppp_bears]))
...
plot(distances, main = "Correlation of Distances Plot")

# Interpret the correlation measure
plot(correlation, main = "Correlation Plot")
```
...
density_bears <- kde2d(ppp_bears$x, ppp_bears$y)
density_parks <- kde2d(ppp_parks$x, ppp_parks$y)

# Plot the bivariate KDE
image(density_bears, col = terrain.colors(20), main = "Bivariate Kernel Density Estimate")
contour(density_bears, add = TRUE, nlevels = 10, col = "black")
points(ppp_bears, pch = 20, col = "red")  # Add bear points
points(ppp_parks, pch = 15, col = "blue") # Add park points
...
cor_matrix_covariates <- cor.im(Elevation = data_covariates$Elevation, Forest = data_covariates$Forest, HFI = data_covariates$HFI, Dist_water = data_covariates$Dist_Water, use = "pairwise.complete.obs") # "everything" did not work
...
cor_plot_bears <- corrplot(cor_matrix_covariates, method = "number", type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) #, add = TRUE)
# cor_plot_bears
```
...
# Fit the PPP model without squared terms
fit_bears_linear <- ppm(ppp_bears ~ Elevation + Forest + HFI + Dist_Water, data = data_covariates)
fit_bears_linear
```
...
$$
\lambda_{bears}(u) = e^{\text{-20.23444} - \text{0.003180172 Elev}(u) + \text{0.0000009420513 Elev}(u)^2 + \text{0.02643186 For}(u) - \text{0.0001907933 For}(u)^2 + \text{9.829718 HFI}(u) - \text{6.926626 HFI}(u)^2 - \text{0.0001073477 Wat}(u) + \text{0.000000001215464 Wat}(u)^2}
$$
...
```{r}
# AIC values
cat("Full model AIC:", AIC(fit_bears), "\n")
cat("Null model AIC:", AIC(fit_null_bears), "\n\n")

# Delta AIC
cat("Difference or Delta AIC:", AIC(fit_null_bears) - AIC(fit_bears))
```

With an AIC of ca. 4854, the extra complexity is well supported by the data.

- The AIC value for the full model (67947.61) is lower than that of the null model (72802.45), indicating that the full model provides a better fit to the data.
- The Delta AIC of 4854.845 suggests that the full model is substantially better than the null model, as a Delta AIC greater than 10 is considered strong evidence in favor of the model with the lower AIC.

```{r}
# Likelihood ratio test comparing full model and null model
...
- This model includes multiple predictors: Elevation, Forest, HFI (Human Footprint Index), and Dist_Water (Distance to Water), along with their squared terms (I(Elevation^2), I(Forest^2), I(HFI^2), and I(Dist_Water^2)). 
- The number of parameters (Npar) for Model 1 is 9. 
- The Deviance indicates how well the model fits the data. 
- The Pr(>Chi) value represents the p-value for the chi-square test of the model's goodness-of-fit. In this case, the p-value is very small (< 2.2e-16), indicating that the model significantly improves the fit compared to a model with no predictors.

Model 2: 

- This is the null model, which only includes an intercept term. 
- The number of parameters for Model 2 is 1. 
- The Deviance of Model 2 is compared to Model 1 using a chi-square test. 
- The -8 in the Degrees of Freedom (Df) column indicates that Model 2 has fewer degrees of freedom compared to Model 1. 
- The large negative Deviance value for Model 2 (-4487.8) and the very small p-value (< 2.2e-16) indicate that Model 1 fits the data significantly better than the null model.

Overall, these results suggest that the predictors included in Model 1 contribute significantly to explaining the variability in the response variable, as Model 1 provides a significantly better fit to the data compared to the null model.
...
```{r}
# The coefficients beside the beta's is without removing NA's before fitting
beta_0 <- fit_bears$coef[1] # -21.02311             # Intercept
beta_1 <- fit_bears$coef[2] # -0.002763701          # Elevation
beta_2 <- fit_bears$coef[3] # 0.0000007955037       # Elevation^2
beta_3 <- fit_bears$coef[4] # 0.03139672            # Forest
beta_4 <- fit_bears$coef[5] # -0.0002180481         # Forest^2
beta_5 <- fit_bears$coef[6] # 11.70025              # HFI
beta_6 <- fit_bears$coef[7] # -8.644236             # HFI^2
beta_7 <- fit_bears$coef[8] # -0.00003768961        # Dist_Water
beta_8 <- fit_bears$coef[9] # -0.000000005277563    # Dist_Water^2
...
cat("Estimated Bear Intensity at 500m elevation, 50% forest coverage, 0.5 HFI, and 10m from water is", answer)
```

**Visualise the fitted model. Note: log scale the estimated intensity when plotting, ignore the standard error. Use the $n$ argument to adjust the resolution.**
```{r fig.height=6, fig.width=8}
...
# Visualise
plot(res_bears,
     main = "Residual Plot of Full-Fitted Model",
     cols = "transparent")
```

### Full Model Fit (selected model)
**Calculate the partial residuals as a function of elevation, forest cover, HFI, and distance from water.**
```{r fig.height=6, fig.width=8}
...
```{r fig.height=6, fig.width=8}
# Calculate the residuals
res_bears_linear <- residuals(fit_bears_linear)

# Visualise
plot(res_bears_linear,
     main = "Residual Plot of Linear-Fitted Model",
     cols = "transparent")
```
...
```{r fig.height=6, fig.width=8}
# Calculate the partial residuals as a function of elevation
par_res_bears_elev <- parres(fit_bears_linear, "Elevation")

# Calculate the relative intensity as a function of forest
par_res_bears_forest <- parres(fit_bears_linear, "Forest")

# Calculate the partial residuals as a function of HFI
par_res_bears_HFI <- parres(fit_bears_linear, "HFI")

# Calculate the relative intensity as a function of Dist_Water
par_res_bears_dist_water <- parres(fit_bears_linear, "Dist_Water")
